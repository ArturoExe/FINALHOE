...
export default function convertToDateType(data) {
...
export function filterByDateOrRange(inputData, startDate, endDate = null) {
  ...
  if (!endDate)
    selectedDates = newData.filter(
      (item) => item.fecha.getTime() === startDate.getTime()
    );
  //Si es na fecha especifica
  else
    selectedDates = newData.filter(
      (item) => item.fecha >= startDate && item.fecha <= endDate
    ); //Si es un rango de fechas (semana, mes)
...
export function dateExistsInSelected(inputDate, selectedDateOrRange) {
  if (selectedDateOrRange.length === 1) {
    ...
    return (
      inputDate.getTime() >= selectedDateOrRange[0].getTime() &&
      inputDate.getTime() <= selectedDateOrRange[1].getTime()
    );
  }
}

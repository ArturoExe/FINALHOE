...
export default function Table() {
  ...
  const [selectedDateOrRange, setSelectedDateOrRange] = useState([
    resetTimeOnDate(new Date()),
  ]);
  ...
  function handleEditModalOpening({
    _id,
    nombre,
    telefono,
    servicio,
    ...
    const convertTo24 = (e) =>
      e ? dayjs(`1/1/1 ${e}`).format("HH:mm:00") : null;
  ...
  function handlePickerChange(picker) {
    if (picker === "date") {
      ...
      setSelectedDateOrRange([
        new Date("January 01, 2023"),
        new Date("December 31, 2023"),
      ]);
    }
  }
  ...
  async function handleDelete(_id) {
    try {
      await fetch("http://localhost:3000/api/appointremove", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      reload("/admin");
      console.log("Delete request successful:", _id);
    } catch (error) {
  ...
  function selectPreviousDay() {
    if (selectedDateOrRange.length === 1) {
      setSelectedDateOrRange((prev) => [
        new Date(prev[0].getTime() - 86400000),
      ]);
    } else if (selectedDateOrRange.length === 2) {
      if (
        selectedDateOrRange[1].getTime() - selectedDateOrRange[0].getTime() <=
        604800000
      ) {
        const { start, end } = getWeek(
          new Date(selectedDateOrRange[0].getTime() - 604800000)
      ...
      <div className="flex">
        ...
        <div className="flex small-gap">
          ...
          >
            <div
              className="btn"
              style={{ width: "16.5rem", justifyContent: "space-between" }}
            >
              <p className="accent-font">
                {dayjs(selectedDateOrRange[0]).format("DD-MM-YYYY")}
                {selectedDateOrRange.length === 2 &&
                  ` | ${dayjs(selectedDateOrRange[1]).format("DD-MM-YYYY")}`}
              </p>
      ...
      <div className="table flex-column">
        ...
        {appointments
          .filter((i) => {
            if (selectedDateOrRange.length === 1) {
              return (
                convertToDateType(i.fecha).getTime() ===
                selectedDateOrRange[0].getTime()
              );
            } else if (selectedDateOrRange.length === 2) {
              return (
                convertToDateType(i.fecha).getTime() >=
            ...
            return (
              <div key={e._id} className="table-item table-row">
                ...
                <Dropdown
                  ...
                  placement="bottom"
                  dropdownRender={() => (
                    <StatusPicker statusPicked={e.estado} item={e} />
                  )}
                >
                  <div>
                    <FontAwesomeIcon
                ...
                <button className="btn btn-small">
                  <FontAwesomeIcon
                    className="icon-secundary"
                    onClick={() => handleEditModalOpening(e)}
                    icon={faPenToSquare}
                  />
                </button>
                <button className="btn btn-small">
        ...
        <div className="modal-container">
          <form
            onSubmit={handleSubmit(
              modalType === "Crear" ? addNewAppointment : updateAppointment
            )}
            ...
            <div className="flex" style={{ alignItems: "flex-start" }}>
              <div className="flex-column column-gap">
                ...
                <input
                  ...
                  {...register("telefono", { required: true, maxLength: 50 })}
                />
                <select {...register("servicio")} defaultValue="Servicio">
                  ...
                  <option value="Depilación con cera">
                    Depilación con cera
                  </option>
                  <option value="Depilación con hilo">
                    Depilación con hilo
                  </option>
                  <option value="Diseño de ceja">Diseño de ceja</option>
                  <option value="Evento">Evento</option>
                  <option value="Maquillaje aerográfico">
                    Maquillaje aerográfico
                  </option>
